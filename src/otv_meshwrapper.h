	...
	public:
		...
		bool LoadFromFileObj(const char* fname, bool loadMtl = true)
		{
			...
			for (int i = 0; i < tiny.materials.size(); ++i) {
				...
				helper::loadimg(textures[i].map_Kd, tiny.materials[i].diffuse_texname, dirpath);
				helper::loadimg(textures[i].map_Ks, tiny.materials[i].specular_texname, dirpath);
				helper::loadimg(textures[i].map_Ns, tiny.materials[i].specular_highlight_texname, dirpath);
				helper::loadimg(textures[i].map_d, tiny.materials[i].alpha_texname, dirpath);
				helper::loadimg(textures[i].map_Bump, tiny.materials[i].bump_texname, dirpath);
			}
			return true;
		}
	};
